...
func (c process) MarshalXML(e *xml.Encoder, _ xml.StartElement) error {
	...
	tokens.startEnvelope(c.Client.soapVersion)
...
func (tokens *tokenData) startEnvelope(ver int8) {
	e := xml.StartElement{
		Name: xml.Name{
			Space: "",
	...
	if customEnvelopeAttrs == nil {
		...
		e.Attr = []xml.Attr{
			...
			{Name: xml.Name{Space: "", Local: soapVer}, Value: soapNS},
		}
	} else {
