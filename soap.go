...
func SoapClientWithConfig(wsdl string, httpClient *http.Client, config *Config) (*Client, error) {
	...
	c := &Client{
		...
		AutoAction:  false,
		soapVersion: 1,                // 增加默认soap版本1.1
		userAgent:   "go-soap client", // 增加默认伪装名称
	}
...
type Client struct {
	...
	wsdl                 string
	config               *Config
	// 增加soap版本,1=1.1, 2=1.2
	soapVersion int8
	userAgent   string
}

// soapVersion设置及返回
func (c *Client) SetSoapVersion(ver int8) int8 {
	if ver == 1 {
		c.soapVersion = 1
	} else {
		c.soapVersion = 2
...
func (c *Client) GetSoapVersion() int8 {
	// 返回 1 或 2
	return c.soapVersion
}

// 伪装代理名称
func (c *Client) SetUserAgent(agentName string) {
...
func (p *process) doRequest(url string) ([]byte, error) {
	...
	req.Header.Add("Content-Type", p.currentVerionContentType())
	req.Header.Add("Accept", "text/xml")
	if p.Client.soapVersion < 2 { // 1.1版本才需要发这个部分
...
func (p *process) currentVerionContentType() string {
	if p.Client.soapVersion < 2 {
